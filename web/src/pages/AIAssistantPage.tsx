...
export const AIAssistantPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'copilot' | 'facts' | 'kb' | 'research' | 'triage' | 'observability'>('copilot');
  ...
  const [chatMessages, setChatMessages] = useState<any[]>([
    {
      role: 'assistant',
      content: 'Hallo! Ich bin Ihr OpenLocalCRM KI-Vertriebs-Copilot. Sie können mich nach Kunden, Pipeline-Analysen, Vor-Ort-Pitches oder Rechtsthemen (UWG § 7, BGB § 355) fragen.',
      time: 'Jetzt',
    },
  ]);
  ...
  const [facts, setFacts] = useState<any[]>([
    { id: 'fact-1', entity: 'Energie Südwest GmbH', field: 'Dachfläche', value: '350 m² Südausrichtung', source: 'Web-Recherche & Luftbild', confidence: 0.94, status: 'BESTÄTIGT' },
    { id: 'fact-2', entity: 'Dr. Michael Weber', field: 'Jahresverbrauch', value: '45.000 kWh Strom', source: 'E-Mail Anfrage (24.08.)', confidence: 0.98, status: 'BESTÄTIGT' },
    { id: 'fact-3', entity: 'Sabine Mustermann', field: 'Heizsystem', value: 'Ölheizung (Bj. 1998)', source: 'D2D Setter-Gespräch', confidence: 0.88, status: 'NEU' },
    { id: 'fact-4', entity: 'Solarpark Rhein-Main', field: 'Zählerkasten', value: '2023 komplett modernisiert', source: 'Vor-Ort Notiz', confidence: 0.92, status: 'NEU' },
  ]);
  ...
  const createKbMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setFeedbackBanner('Wissensdokument erfolgreich in pgvector vektorisiert (384-dim Embeddings gespeichert)!');
  ...
  const createResearchJobMutation = useMutation({
    ...
    onSuccess: (newJob: any) => {
      ...
      setFeedbackBanner(`Deep Research für "${newJob.query}" erfolgreich abgeschlossen & analysiert!`);
      setTimeout(() => setFeedbackBanner(null), 4000);
    },
  });
  ...
  const recentLogs = [
    ...
    },
    {
      ...
      extracted_decision: {
        ...
        sentiment: 'NEUTRAL',
        priority: 'MEDIUM',
        summary: 'Empfehlung: Vor-Ort-Termin zur Dachbegehung vorschlagen, da Zählerkasten 2023 bereits erneuert wurde.',
    ...
    },
    {
      ...
      input_prompt: 'Absender: meier@baubetrieb.de\nBetreff: Wechselrichter Störung\nNachricht: Wechselrichter speist nicht mehr ein!',
      extracted_decision: {
        category: 'REKLAMATION',
        sentiment: 'NEGATIVE',
        priority: 'URGENT',
        summary: 'Störungsmeldung Wechselrichter Ausfall.',
        suggested_draft: 'Sehr geehrter Herr Meier, wir haben Ihre Störungsmeldung erfasst. Unser Service meldet sich binnen 2 Stunden...',
      },
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400">
            Gemma 12B Assistenz, Evidence-Ledger, Wissensbasis, Web-Recherche und EU AI Act Observability
          </p>
        </div>
      ...
      {activeTab === 'copilot' && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl h-[650px] flex flex-col overflow-hidden shadow-sm">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-950/40">
            {chatMessages.map((msg, i) => (
              <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                >
                  {msg.content}
                  <div className={`text-[10px] mt-1.5 text-right ${msg.role === 'user' ? 'text-emerald-950/70' : 'text-slate-500'}`}>
                    {msg.time}
                  </div>
                </div>
              </div>
          ...
          <div className="px-4 py-2.5 bg-slate-900 border-t border-slate-800 flex gap-2 overflow-x-auto text-xs">
            {['Pipeline-Status analysieren', 'Pitch für 10 kWp Photovoltaik erstellen', 'Widerrufsfrist nach § 355 BGB prüfen', 'Follow-Up E-Mail an Gewerbekunden'].map((chip, idx) => (
              <button
                key={idx}
          ...
          {/* Input Bar */}
          <form onSubmit={handleSendChat} className="p-4 bg-slate-950 border-t border-slate-800 flex gap-3">
            <input
              type="text"
      ...
      {activeTab === 'facts' && (
        <div className="space-y-4">
          <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl flex items-center justify-between">
            <div>
              <h3 className="font-bold text-slate-100 text-sm">Evidence-Ledger & Gewichtete Fakten (§5.2)</h3>
              <p className="text-xs text-slate-400">Automatisch aus E-Mails, Telefonaten und Web-Recherchen extrahierte Kunden- und Energiedaten</p>
            </div>
          ...
          <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-400">
                Unternehmensspezifisches Wissen, Preismodelle und technische Datenblätter für den semantischen RAG-Retrieval
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredKbArticles.length === 0 ? (
              ...
              filteredKbArticles.map((entry) => (
                <div key={entry.id} className="p-5 bg-slate-900 border border-slate-800 rounded-2xl space-y-3 shadow-sm hover:border-slate-700 transition-colors">
                  ...
                  <div className="pt-2 border-t border-slate-800 flex items-center justify-between text-[11px] text-slate-400">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="truncate max-w-[160px]">{entry.source || 'Manuelle Eingabe'}</span>
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-sm space-y-4">
            ...
            <p className="text-xs text-slate-400 leading-relaxed">
              Automatisierte Web-Recherche per Gemma 12B & SSRF-geschütztem Web-Crawl. Analysiert Impressum, Gewerbedachflächen, Entscheider und technologische Eignung.
            </p>

            <form onSubmit={handleStartResearchJob} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Unternehmens-Domain oder Name *</label>
                <div className="relative">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Recherche-Tiefe</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Fokus & Zielbranche</label>
                <select
                ...
                >
                  <option value="Gewerbesolar & Hallendach">Gewerbesolar & Hallendach (30–500 kWp)</option>
                  <option value="B2C Eigenheim & Dachsanierung">B2C Eigenheim & Dachsanierung</option>
              ...
              >
                <Sparkles className={`w-4 h-4 ${createResearchJobMutation.isPending ? 'animate-spin' : ''}`} />
                <span>{createResearchJobMutation.isPending ? 'Research-Auftrag läuft...' : 'Automatisch Recherchieren'}</span>
              </button>
            </form>
                ...
                {researchJobs.map((job) => (
                  ...
                  >
                    <div className="truncate pr-2">
                      <div className="font-semibold text-slate-200 truncate">{job.company_name || job.query}</div>
            ...
            {selectedResearchJob ? (
              <div className="space-y-4">
                <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 space-y-1">
                  <div className="text-[10px] text-slate-500 uppercase font-semibold">Identifiziertes Unternehmen & Domain</div>
                  <div className="text-sm font-bold text-slate-100">{selectedResearchJob.result?.site_title || selectedResearchJob.company_name}</div>
                  <div className="text-xs text-slate-400 font-mono">{selectedResearchJob.query}</div>
                </div>

                <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 space-y-1.5">
                  <div className="text-[10px] text-slate-500 uppercase font-semibold">Geschäftsmodell & PV-Eignung</div>
                  <p className="text-xs text-slate-300 leading-relaxed">{selectedResearchJob.result?.summary}</p>
                </div>

                {/* Decision Makers */}
                {selectedResearchJob.result?.decision_makers && (
                  <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 space-y-2">
                    <div className="text-[10px] text-slate-500 uppercase font-semibold">Gefundene Entscheider & Kontakte</div>
                    <div className="flex flex-wrap gap-2">
                      {selectedResearchJob.result.decision_makers.map((person: string, idx: number) => (
                ...
                {selectedResearchJob.result?.industry_tags && (
                  <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 space-y-2">
                    <div className="text-[10px] text-slate-500 uppercase font-semibold">Branchen- & Eignungs-Tags</div>
                    <div className="flex flex-wrap gap-1.5">
                      {selectedResearchJob.result.industry_tags.map((kw: string, idx: number) => (
                        <span key={idx} className="px-2.5 py-1 rounded-lg bg-slate-900 border border-slate-700 text-[11px] text-emerald-400 font-medium">
                          {kw}
                        </span>
                      ))}
                    </div>
                ...
                <div className="pt-3 border-t border-slate-800 flex justify-end gap-3">
                  <button
                    onClick={() => {
                      setFeedbackBanner(`Recherche-Dossier für "${selectedResearchJob.company_name}" als CRM-Notiz abgelegt!`);
                      setTimeout(() => setFeedbackBanner(null), 4000);
                    }}
                  ...
                  <button
                    onClick={() => {
                      setFeedbackBanner(`Unternehmen "${selectedResearchJob.company_name}" erfolgreich ins CRM übernommen!`);
                      setTimeout(() => setFeedbackBanner(null), 4000);
                    }}
              ...
              <div className="h-64 flex flex-col items-center justify-center text-center text-slate-500 space-y-2">
                ...
                <div className="text-xs">Wähle links einen Auftrag aus oder starte eine neue Recherche.</div>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-slate-100 text-base">Neues Wissensdokument ablegen (RAG)</h3>
              </div>
              <button onClick={() => setIsKbModalOpen(false)} className="text-slate-400 hover:text-slate-200">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            >
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Titel des Dokuments *</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">Quelldatei / Referenz</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Wissensinhalt / Fließtext *</label>
                <textarea
                  required
                  rows={5}
      ...
      {activeTab === 'triage' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <span className="text-sm font-bold text-slate-200">Strukturierte KI-Ergebnisse (Gemma 12B)</span>
              {triageResult && (
                <span className="text-xs px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-semibold">
                  Validiert
                </span>
            ...
            {triageResult ? (
              <div className="space-y-4">
                <div className="grid grid-cols-3 gap-3">
                  <div className="bg-slate-950 border border-slate-800 rounded-xl p-3">
                    <div className="text-[10px] text-slate-500 uppercase font-semibold">Kategorie</div>
                    <div className="text-xs font-bold text-emerald-400 mt-1">{triageResult.category}</div>
                  </div>
                  <div className="bg-slate-950 border border-slate-800 rounded-xl p-3">
                    <div className="text-[10px] text-slate-500 uppercase font-semibold">Priorität</div>
                    <div className="text-xs font-bold text-amber-400 mt-1">{triageResult.priority}</div>
                  </div>
                  <div className="bg-slate-950 border border-slate-800 rounded-xl p-3">
                    <div className="text-[10px] text-slate-500 uppercase font-semibold">Sentiment</div>
                    <div className="text-xs font-bold text-cyan-400 mt-1">{triageResult.sentiment}</div>
                  </div>
                </div>

                <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 space-y-1.5">
                  <div className="text-[11px] font-bold text-slate-400 uppercase">Zusammenfassung</div>
              ...
              <div className="h-64 flex flex-col items-center justify-center text-center text-slate-500 space-y-2">
                ...
                <div className="text-xs">Klicke links auf "Mit KI-Engine analysieren", um die Extraktion zu starten.</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-2">
              ...
              <div className="text-2xl font-bold text-slate-100">{stats.average_latency_ms.toFixed(0)} ms</div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm">
            <div className="p-4 border-b border-slate-800 flex items-center justify-between">
              <div>
                <h3 className="font-bold text-slate-200 text-sm">EU AI Act Revisionssicheres Audit-Log & KI-Entscheidungs-Inspektor</h3>
                <p className="text-xs text-slate-400">Klicken Sie auf einen Log-Eintrag, um Eingabe-Prompts, PII-Maskierungen und KI-Klassifikationen im Detail zu prüfen.</p>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-slate-800">
                  {recentLogs.map((log: any) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 font-semibold text-slate-100">{log.interaction_type}</td>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-3xl w-full p-6 shadow-2xl space-y-5 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-slate-100 text-base">KI-Audit & Entscheidungs-Protokoll</h3>
                  <div className="text-xs text-slate-400 font-mono">Job-ID: {selectedAuditLog.id} • {selectedAuditLog.interaction_type}</div>
                </div>
              </div>
              <button 
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-950 p-3.5 rounded-xl border border-slate-800 text-xs">
              <div>
                <span className="text-slate-500 block text-[10px] uppercase font-semibold">Modell</span>
                <span className="font-mono text-emerald-400 font-bold">{selectedAuditLog.model_name}</span>
              </div>
              <div>
                <span className="text-slate-500 block text-[10px] uppercase font-semibold">Inferenz-Latenz</span>
                <span className="font-mono text-slate-200">{selectedAuditLog.latency_ms} ms</span>
              </div>
              <div>
                <span className="text-slate-500 block text-[10px] uppercase font-semibold">Infrastruktur</span>
                <span className="text-slate-200 font-medium">100% On-Premise</span>
              </div>
              <div>
                <span className="text-slate-500 block text-[10px] uppercase font-semibold">EU AI Act Status</span>
            ...
            <div className="space-y-1.5">
              ...
              {selectedAuditLog.redacted_items?.length > 0 && (
                <div className="p-2.5 bg-slate-950 border border-amber-500/20 rounded-xl space-y-1">
                  <div className="text-[10px] text-slate-500 uppercase font-semibold">Maskierte Datenpunkte vor LLM-Übergabe:</div>
                  <div className="flex flex-wrap gap-1.5">
                    {selectedAuditLog.redacted_items.map((item: string, idx: number) => (
                      <span key={idx} className="font-mono text-[11px] px-2 py-0.5 rounded bg-amber-500/10 text-amber-300 border border-amber-500/30">
                        {item}
                      </span>
                    ))}
                  </div>
            ...
            <div className="space-y-1.5">
              <div className="text-xs font-bold text-slate-300">📥 Bereinigter Eingabe-Prompt (Input-Kontext)</div>
            ...
            <div className="space-y-1.5">
              <div className="text-xs font-bold text-slate-300">🧠 Extrahierte KI-Entscheidung & Handlungs-Vorschlag</div>
              <div className="p-3.5 bg-slate-950 border border-slate-800 rounded-xl space-y-2.5">
                <div className="grid grid-cols-3 gap-2">
                  <div className="bg-slate-900 p-2 rounded-lg border border-slate-800">
                    <span className="text-[10px] text-slate-500 uppercase block font-semibold">Klassifikation</span>
                    <span className="font-bold text-emerald-400 text-xs">{selectedAuditLog.extracted_decision.category}</span>
                  </div>
                  <div className="bg-slate-900 p-2 rounded-lg border border-slate-800">
                    <span className="text-[10px] text-slate-500 uppercase block font-semibold">Dringlichkeit</span>
                    <span className="font-bold text-amber-400 text-xs">{selectedAuditLog.extracted_decision.priority}</span>
                  </div>
                  <div className="bg-slate-900 p-2 rounded-lg border border-slate-800">
                    <span className="text-[10px] text-slate-500 uppercase block font-semibold">Stimmung</span>
                    <span className="font-bold text-cyan-400 text-xs">{selectedAuditLog.extracted_decision.sentiment}</span>
                  </div>
                </div>

                <div>
                  <span className="text-[10px] text-slate-500 uppercase block font-semibold mb-0.5">Erkannte Zusammenfassung</span>
                  <p className="text-xs text-slate-200">{selectedAuditLog.extracted_decision.summary}</p>
                </div>

                <div>
                  <span className="text-[10px] text-slate-500 uppercase block font-semibold mb-0.5">Vorbereitete Aktion / Entwurf (HITL)</span>
